...
public class TimeModel{
	...
	@Inject
	protected TimeModel(final EventBus eventBus) {
		this.eventBus = eventBus;
		Scheduler.get().scheduleFixedPeriod(new RepeatingCommand() {
			@Override public boolean execute() {
				eventBus.fireEventFromSource(new TimeChangeEvent(), TimeModel.this);
				updateStatus();
				return true;
			}
		}, 1*1000);
	...
	private void updateStatus(){
		if(wasWorking!=isWorking() || firstUpdate){
			...
			eventBus.fireEventFromSource(new WorkChangeEvent(), TimeModel.this);
			firstUpdate = false;
		}
	}
